<%@ page import="com.mydata.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.mydata.crm.workbench.domain.Tran" %>
        ...
        $.ajax({
            ...
            success:function (data) {
                if (data.success){
                    ...
                    //changeIcon(stage,index);//肯定用自己写的呀
                }
...
<body>
		...
		<%
			...
			if ("0".equals(currentPossibility)){
				for (int i=0;i<dvList.size();i++){
					...
					System.out.println("listStage====="+listStage);
					if ("0".equals(listPossibility)){
					    //表示后两个阶段
					    if (listStage.equals(currentStage)){
					        //红×
							%>
							<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-record mystage" data-toggle="popover" data-placement="bottom" data-content="<%=listStage%>" style="color: #FF0000;"></span>
							-------------
							<%
						}else {
					        //黑×
							%>
							<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-remove mystage" data-toggle="popover" data-placement="bottom" data-content="<%=listStage%>" style="color: #000000;"></span>
							-----------
							<%
						}
					}else{
				...
				for (int i=0;i<dvList.size();i++) {
					...
					if ("0".equals(listPossibility)){
					    ...
					    if (i==index){
							//绿色标记
							%>
							<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage"  data-toggle="popover" data-placement="bottom" data-content="<%=listStage%>" style="color: #90F790;"></span>
							-----------
							<%
						}else if(i<index){
					        //绿色圈圈
							%>
							<span id="<%=i%>"  onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover" data-placement="bottom" data-content="<%=listStage%>" style="color: #90F790;"></span>
							-----------
							<%
						}else{
							//黑色圈圈
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 50px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${t.customerId}&nbsp;</b></div>
